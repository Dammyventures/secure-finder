  ...
  return (
    <group ref={galaxyRef}>
      <Points ref={pointsRef} positions={particlePositions} colors={colorPositions} stride={3}>
        <PointMaterial transparent opacity={0.6} size={0.025} sizeAttenuation blending={2} depthWrite={false} />
      </Points>
      <Float speed={1.2} rotationIntensity={0.5} floatIntensity={0.7}>
        <Sphere args={[0.6, 64, 64]} position={[0,0,0]}>
          <MeshDistortMaterial color="#1C448E" distort={0.2} speed={1.5} roughness={0.05} metalness={0.95} emissive="#938BA1" emissiveIntensity={0.7} transparent opacity={0.5} />
        </Sphere>
      </Float>
      <Ring args={[0.9, 1.1, 64]} position={[0,0,0]} rotation={[Math.PI/3,0.2,0]}>
        <meshStandardMaterial color="#F4FDFF" emissive="#1C448E" emissiveIntensity={0.2} metalness={0.9} transparent opacity={0.3} />
      </Ring>
      {[1,2].map((i) => {
        const radius = 1.4 + i * 0.4
        return (
          <Float key={i} speed={0.3 + i*0.1} rotationIntensity={0.2} floatIntensity={0.3}>
            <Ring args={[radius, radius + 0.03, 80]} position={[0,0,0]} rotation={[Math.PI/2 + i*0.3, i*0.3, 0]}>
              <meshStandardMaterial color={i===1 ? "#938BA1" : "#1C448E"} emissive={i===1 ? "#F4FDFF" : "#938BA1"} emissiveIntensity={0.1} metalness={0.8} transparent opacity={0.15} wireframe={i===2} />
            </Ring>
          </Float>
        )
      })}
      {[...Array(8)].map((_, i) => {
        const angle = (i/8)*Math.PI*2; const radius = 1.8 + Math.random()*1.5; const height = (Math.random()-0.5)*2
        return (
          <Float key={i+100} speed={0.4 + Math.random()*0.3} rotationIntensity={0.8} floatIntensity={0.8} position={[Math.cos(angle+i*0.3)*radius, height, Math.sin(angle+i*0.3)*radius]}>
            <Sphere args={[0.04 + Math.random()*0.04, 16, 16]}>
              <MeshDistortMaterial color={i%2===0 ? "#F4FDFF" : "#938BA1"} distort={0.4} speed={2} metalness={0.9} emissive={i%2===0 ? "#F4FDFF" : "#938BA1"} emissiveIntensity={0.3} />
            </Sphere>
          </Float>
        )
      })}
...
const FluidAuraBackground: React.FC = () => {
  ...
  return (
    <group ref={groupRef}>
      {[...Array(4)].map((_, i) => {
        const size = 2 + i * 1.8; const opacity = 0.025 - i * 0.005
        return (
          <Float key={i} speed={0.2 + i*0.03} rotationIntensity={0.08} floatIntensity={0.2 + i*0.03} position={[0, -0.3 + i*0.2, -1.5 - i*0.4]}>
            <Sphere args={[size, 32, 32]}>
              <MeshDistortMaterial color={i%2===0 ? "#1C448E" : "#938BA1"} distort={0.5 + i*0.06} speed={0.3 + i*0.03} roughness={0.3} metalness={0.2} transparent opacity={opacity} emissive={i%2===0 ? "#1C448E" : "#938BA1"} emissiveIntensity={0.06} />
            </Sphere>
          </Float>
        )
      })}
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-[#1C448E] via-[#0F2A5E] to-[#1C448E] overflow-hidden">
      ...
      <div className="relative z-10 px-4 md:px-6 lg:px-8 py-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[#F4FDFF]/50 text-sm">
                      You've helped reunite {stats.itemsClaimed} items. Great job! 🎉
                    </p>
                  </div>
                </div>
        ...
        {/* Header */}
        <motion.div style={{ opacity: headerOpacity }} className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
          <div>
            <motion.h1 initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-[#F4FDFF] to-[#938BA1] bg-clip-text text-transparent">
              Dashboard
            </motion.h1>
            <motion.p initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="text-[#F4FDFF]/40 text-sm mt-1">
              Overview of your activity and statistics
            </motion.p>
          </div>
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 flex-wrap">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="outline" className="border-[#F4FDFF]/20 text-[#F4FDFF] hover:bg-[#F4FDFF]/10">
                <Bell size={18} className="mr-2" />
                Notifications
                {notificationCount > 0 && (
                  <span className="ml-2 w-5 h-5 bg-[#938BA1] rounded-full text-xs flex items-center justify-center text-[#1C448E]">{notificationCount}</span>
                )}
              </Button>
            </motion.div>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        {!user?.identityVerified && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-5 bg-[#938BA1]/10 backdrop-blur-2xl rounded-2xl border border-[#938BA1]/20">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#F4FDFF]/50 text-sm">Verify your identity to unlock full platform features and increase trust score</p>
                </div>
              </div>
              <Link to="/verify">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-8">
          <AnimatedStatCard icon={<Package size={24} className="text-white" />} title="Items Reported" value={stats.itemsReported} color="from-[#1C448E] to-[#938BA1]" delay={0.1} link="/my-items" linkText="View all items" />
          <AnimatedStatCard icon={<CheckCircle size={24} className="text-white" />} title="Items Claimed" value={stats.itemsClaimed} color="from-[#938BA1] to-[#1C448E]" delay={0.2} link="/my-claims" linkText="View claims" />
          <AnimatedStatCard icon={<DollarSign size={24} className="text-white" />} title="Rewards Earned" value={`$${stats.rewardsEarned}`} color="from-[#F4FDFF] to-[#938BA1]" delay={0.3} link="/rewards" linkText="Claim rewards" />
          <AnimatedStatCard icon={<Award size={24} className="text-white" />} title="Verification Score" value={`${stats.verificationScore}%`} color={getVerificationColor()} delay={0.4} link="/verify" linkText={user?.identityVerified ? 'Upgrade now' : 'Start verification'} />
        </div>

        {/* Stats row 2 */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
          <AnimatedStatCard icon={<Clock size={24} className="text-white" />} title="Active Claims" value={stats.activeClaims} color="from-[#1C448E] to-[#0F2A5E]" delay={0.5} link="/claims" linkText="Manage claims" />
          <AnimatedStatCard icon={<Activity size={24} className="text-white" />} title="Avg Resolution" value={`${stats.avgResolutionTime}h`} color="from-[#938BA1] to-[#1C448E]" delay={0.6} link="/analytics" linkText="View analytics" />
          <AnimatedStatCard icon={<Users size={24} className="text-white" />} title="Community Impact" value="+245" trend="+12%" trendDirection="up" color="from-[#F4FDFF] to-[#938BA1]" delay={0.7} link="/community" linkText="See impact" />
        </div>

        {/* Quick Actions & Recent Items */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Quick Actions */}
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="lg:col-span-1">
            <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-2xl p-6 border border-[#F4FDFF]/10">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <motion.div key={idx} whileHover={{ scale: 1.02, x: 5 }} whileTap={{ scale: 0.98 }}>
                    <Link to={action.path}>
          ...
          {/* Recent Items */}
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className="lg:col-span-2">
            <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-2xl p-6 border border-[#F4FDFF]/10">
              ...
              {recentItems.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {recentItems.map((item, idx) => (
                    <motion.div key={item.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1 + idx * 0.1 }}>
                      <ItemCard item={item} />
                    </motion.div>
                  ))}
                </div>
        ...
        {/* Footer info boxes */}
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 mb-8">
          <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-2xl p-6 border border-[#F4FDFF]/10">
            ...
            <div className="space-y-3">
              {[
                ...
                <motion.div key={idx} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.1 + idx * 0.1 }} className="flex justify-between items-center py-2 border-b border-[#F4FDFF]/10">
          ...
          <div className="bg-gradient-to-br from-[#1C448E]/20 to-[#938BA1]/20 backdrop-blur-2xl rounded-2xl p-6 border border-[#F4FDFF]/10">
            ...
            <ul className="space-y-3">
              {[
                ...
                <motion.li key={idx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.1 + idx * 0.1 }} className="flex items-start gap-2 text-[#F4FDFF]/60 text-sm">
            ...
            <div className="mt-4 pt-3 flex items-center justify-between">
              <span className="text-xs text-[#F4FDFF]/30">✓ Trust Score: {user?.identityVerified ? 'High' : 'Medium'}</span>
