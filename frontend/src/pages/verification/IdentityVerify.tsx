...
const VerificationParticleGalaxy: React.FC = () => {
  ...
  const [colorPositions] = useState(() => {
    ...
    const palette = [
      [0.956, 0.992, 1.0], [0.11, 0.267, 0.557], [0.576, 0.545, 0.631], [0.956, 0.992, 1.0]
    ]
    for (let i = 0; i < 2000; i++) {
  ...
  return (
    <group ref={galaxyRef}>
      <Points ref={pointsRef} positions={particlePositions} colors={colorPositions} stride={3}>
        <PointMaterial transparent opacity={0.6} size={0.028} sizeAttenuation blending={2} depthWrite={false} />
      </Points>
      <Float speed={1.3} rotationIntensity={0.5} floatIntensity={0.8}>
        <Sphere args={[0.65,64,64]} position={[0,0,0]}>
          <MeshDistortMaterial color="#1C448E" distort={0.15} speed={1.2} roughness={0.05} metalness={0.95} emissive="#938BA1" emissiveIntensity={0.8} transparent opacity={0.55} />
        </Sphere>
      </Float>
      <Ring args={[0.95,1.15,64]} position={[0,0,0]} rotation={[Math.PI/3.5,0.25,0]}>
        <meshStandardMaterial color="#F4FDFF" emissive="#1C448E" emissiveIntensity={0.25} metalness={0.9} transparent opacity={0.35} />
      </Ring>
      {[1,2].map((i) => {
        const radius = 1.3 + i*0.45
        return (
          <Float key={i} speed={0.35+i*0.1} rotationIntensity={0.25} floatIntensity={0.35}>
            <Ring args={[radius, radius+0.04,80]} position={[0,0,0]} rotation={[Math.PI/2.5+i*0.35, i*0.35,0]}>
              <meshStandardMaterial color={i===1?"#938BA1":"#1C448E"} emissive={i===1?"#F4FDFF":"#938BA1"} emissiveIntensity={0.12} metalness={0.8} transparent opacity={0.2} wireframe={i===2} />
            </Ring>
          </Float>
        )
      })}
      {[...Array(8)].map((_,i)=>{
        const angle=(i/8)*Math.PI*2; const radius=1.7+Math.random()*1.6; const height=(Math.random()-0.5)*2.2
        return (
          <Float key={i+100} speed={0.5+Math.random()*0.35} rotationIntensity={0.9} floatIntensity={0.9} position={[Math.cos(angle+i*0.35)*radius, height, Math.sin(angle+i*0.35)*radius]}>
            <Sphere args={[0.045+Math.random()*0.045,16,16]}>
              <MeshDistortMaterial color={i%2===0?"#F4FDFF":"#938BA1"} distort={0.5} speed={2.2} metalness={0.9} emissive={i%2===0?"#F4FDFF":"#938BA1"} emissiveIntensity={0.35} />
            </Sphere>
          </Float>
        )
      })}
...
const FluidAuraBackground: React.FC = () => {
  ...
  return (
    <group ref={groupRef}>
      {[...Array(4)].map((_,i)=>{
        const size=2.2+i*1.9; const opacity=0.028-i*0.005
        return (
          <Float key={i} speed={0.22+i*0.035} rotationIntensity={0.08} floatIntensity={0.22+i*0.035} position={[0, -0.35+i*0.22, -1.8-i*0.45]}>
            <Sphere args={[size,32,32]}>
              <MeshDistortMaterial color={i%2===0?"#1C448E":"#938BA1"} distort={0.55+i*0.07} speed={0.35+i*0.035} roughness={0.3} metalness={0.2} transparent opacity={opacity} emissive={i%2===0?"#1C448E":"#938BA1"} emissiveIntensity={0.07} />
            </Sphere>
          </Float>
        )
      })}
...
const IdentityVerify: React.FC = () => {
  ...
  const {
    ...
    defaultValues: { identityType: 'passport' },
  })
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-6xl">
        ...
        {currentStep !== 'success' && currentStep !== 'failed' && (
          <motion.div variants={itemVariants} className="mb-8 max-w-2xl mx-auto">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-medium text-[#F4FDFF]/40">Step {currentStepConfig.stepNumber} of {currentStepConfig.totalSteps}</span>
              <span className="text-xs font-medium text-[#F4FDFF]/40">{Math.round((currentStepConfig.stepNumber / currentStepConfig.totalSteps) * 100)}%</span>
            </div>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
            ...
            <div className="max-w-4xl mx-auto">
              <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-3xl p-8 border border-[#F4FDFF]/10">
                <div className="text-center mb-8">
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.2 }} className="w-28 h-28 bg-gradient-to-r from-[#1C448E]/20 to-[#938BA1]/20 rounded-full flex items-center justify-center mx-auto mb-6">
                    <Shield className="h-14 w-14 text-[#F4FDFF]" />
                  </motion.div>
                  <h2 className="text-2xl font-bold text-[#F4FDFF] mb-4">Secure Identity Verification</h2>
                  <p className="text-[#F4FDFF]/50 mb-6">Verify your identity to access all features. This helps prevent fraud and ensures a safe community.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  {verificationBenefits.map((benefit, idx) => (
                    <motion.div key={idx} variants={itemVariants} whileHover={{ x: 5 }} className="flex items-start gap-3 p-4 bg-[#F4FDFF]/5 rounded-xl border border-[#F4FDFF]/10 hover:border-[#F4FDFF]/20 transition-all">
                      <div className="flex-shrink-0"><div className="w-10 h-10 bg-gradient-to-r from-[#1C448E]/20 to-[#938BA1]/20 rounded-lg flex items-center justify-center">{benefit.icon}</div></div>
                      <div><h3 className="font-semibold text-[#F4FDFF]">{benefit.title}</h3><p className="text-xs text-[#F4FDFF]/40">{benefit.description}</p></div>
                    </motion.div>
                  ))}
                </div>

                <div className="bg-gradient-to-r from-[#1C448E]/20 to-[#938BA1]/20 border border-[#F4FDFF]/10 rounded-xl p-4 mb-8">
                  <div className="flex items-start gap-3"><Lock className="h-5 w-5 text-[#938BA1] mt-0.5" /><div><h4 className="font-medium text-[#F4FDFF] mb-1">Your Data is Secure</h4><p className="text-xs text-[#F4FDFF]/40">All documents are encrypted and processed securely. We comply with GDPR and never share your personal information with third parties.</p></div></div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Button onClick={startVerification} size="lg" className="bg-gradient-to-r from-[#F4FDFF] to-[#938BA1] text-[#1C448E] font-semibold px-8 hover:shadow-lg hover:shadow-[#938BA1]/20" isLoading={isLoading}>
                      Start Verification <Sparkles className="ml-2 h-4 w-4" />
                    </Button>
                  </motion.div>
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Button onClick={() => navigate('/dashboard')} variant="outline" size="lg" className="border-[#F4FDFF]/20 text-[#F4FDFF] hover:bg-[#F4FDFF]/10">Verify Later</Button>
                  </motion.div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-3xl p-8 border border-[#F4FDFF]/10">
                ...
                <form onSubmit={handleSubmitPersonalInfo(submitPersonalInfo)} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className="block text-sm font-medium text-[#F4FDFF]/70 mb-2">City *</label><input type="text" {...registerPersonalInfo('city')} className="w-full px-4 py-3 bg-[#F4FDFF]/5 border border-[#F4FDFF]/15 rounded-xl text-[#F4FDFF] placeholder-[#F4FDFF]/20 focus:border-[#F4FDFF] focus:ring-2 focus:ring-[#F4FDFF]/20 transition-all outline-none" placeholder="New York" />{personalInfoErrors.city && <p className="text-[#938BA1] text-xs mt-1">{personalInfoErrors.city.message}</p>}</div>
                    <div><label className="block text-sm font-medium text-[#F4FDFF]/70 mb-2">Postal Code *</label><input type="text" {...registerPersonalInfo('postalCode')} className="w-full px-4 py-3 bg-[#F4FDFF]/5 border border-[#F4FDFF]/15 rounded-xl text-[#F4FDFF] placeholder-[#F4FDFF]/20 focus:border-[#F4FDFF] focus:ring-2 focus:ring-[#F4FDFF]/20 transition-all outline-none" placeholder="10001" />{personalInfoErrors.postalCode && <p className="text-[#938BA1] text-xs mt-1">{personalInfoErrors.postalCode.message}</p>}</div>
                    <div><label className="block text-sm font-medium text-[#F4FDFF]/70 mb-2">Country *</label><input type="text" {...registerPersonalInfo('country')} className="w-full px-4 py-3 bg-[#F4FDFF]/5 border border-[#F4FDFF]/15 rounded-xl text-[#F4FDFF] placeholder-[#F4FDFF]/20 focus:border-[#F4FDFF] focus:ring-2 focus:ring-[#F4FDFF]/20 transition-all outline-none" placeholder="United States" />{personalInfoErrors.country && <p className="text-[#938BA1] text-xs mt-1">{personalInfoErrors.country.message}</p>}</div>
                  </div>
                  <div className="flex justify-between pt-6">
                    <Button variant="outline" onClick={() => setCurrentStep('welcome')} className="border-[#F4FDFF]/20 text-[#F4FDFF] hover:bg-[#F4FDFF]/10">Back</Button>
                    <Button type="submit" isLoading={isLoading} className="bg-gradient-to-r from-[#F4FDFF] to-[#938BA1] text-[#1C448E] hover:shadow-lg hover:shadow-[#938BA1]/20">Continue <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  </div>
                </form>
              </div>
            </div>
          ...
          {currentStep === 'document_select' && (
            <div className="max-w-3xl mx-auto">
              <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-3xl p-8 border border-[#F4FDFF]/10">
                <div className="mb-6"><h2 className="text-xl font-bold text-[#F4FDFF] mb-2">Select Document Type</h2><p className="text-[#F4FDFF]/40 text-sm">Choose the type of identity document you want to use for verification.</p></div>
                <form onSubmit={handleSubmitDocument(submitDocumentType)}>
                  <div className="space-y-3 mb-8">
                    {documentTypes.map((docType) => (
                      <motion.label key={docType.id} variants={itemVariants} whileHover={{ x: 5 }} className={`flex items-start p-4 border-2 rounded-xl cursor-pointer transition-all ${selectedDocumentType === docType.id ? 'border-[#F4FDFF] bg-[#F4FDFF]/10' : 'border-[#F4FDFF]/15 hover:border-[#F4FDFF]/30'}`}>
                        <input type="radio" value={docType.id} {...registerDocument('identityType')} className="mt-1 w-4 h-4 text-[#938BA1] focus:ring-[#F4FDFF]/20" />
                        <div className="ml-3 flex-1">
                          <div className="flex items-center gap-3"><span className="text-3xl">{docType.icon}</span><div><h3 className="font-bold text-[#F4FDFF]">{docType.name}</h3><p className="text-sm text-[#F4FDFF]/40">{docType.description}</p></div></div>
                          <div className="mt-3 ml-9"><p className="text-xs font-medium text-[#F4FDFF]/50 mb-2">Requirements:</p><ul className="text-xs text-[#F4FDFF]/30 space-y-1">{docType.requirements.map((req, idx) => (<li key={idx} className="flex items-center"><CheckCircle className="h-3 w-3 text-[#938BA1] mr-2" />{req}</li>))}</ul></div>
                        </div>
                      </motion.label>
                    ))}
                  </div>
                  {documentErrors.identityType && <div className="mb-6 p-3 bg-[#938BA1]/10 border border-[#938BA1]/20 rounded-xl"><p className="text-sm text-[#938BA1]">{documentErrors.identityType.message}</p></div>}
                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setCurrentStep('personal_info')} className="border-[#F4FDFF]/20 text-[#F4FDFF] hover:bg-[#F4FDFF]/10">Back</Button>
                    <Button type="submit" className="bg-gradient-to-r from-[#F4FDFF] to-[#938BA1] text-[#1C448E] hover:shadow-lg hover:shadow-[#938BA1]/20">Continue <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  </div>
                </form>
              </div>
            </div>
          ...
          {currentStep === 'success' && (
            <div className="max-w-md mx-auto">
              <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-3xl p-8 border border-[#F4FDFF]/10 text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} className="w-24 h-24 bg-gradient-to-r from-[#938BA1]/20 to-[#F4FDFF]/20 rounded-full flex items-center justify-center mx-auto mb-6"><CheckCircle className="h-12 w-12 text-[#938BA1]" /></motion.div>
          ...
          {currentStep === 'failed' && (
            <div className="max-w-md mx-auto">
              <div className="bg-[#F4FDFF]/5 backdrop-blur-2xl rounded-3xl p-8 border border-[#F4FDFF]/10 text-center">
                <div className="w-24 h-24 bg-gradient-to-r from-[#938BA1]/20 to-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6"><XCircle className="h-12 w-12 text-[#938BA1]" /></div>
                <h2 className="text-2xl font-bold text-[#F4FDFF] mb-2">Verification Failed</h2>
                <p className="text-[#F4FDFF]/50 mb-4">{verificationStatus.message || 'We could not verify your identity.'}</p>
                <div className="bg-[#938BA1]/10 border border-[#938BA1]/20 rounded-xl p-4 mb-6 text-left"><h4 className="font-medium text-[#938BA1] mb-2">Possible reasons:</h4><ul className="text-sm text-[#F4FDFF]/50 space-y-1"><li>• Documents are blurry or unclear</li><li>• Information mismatch</li><li>• Expired document</li><li>• Suspicious activity detected</li></ul></div>
                <div className="space-y-3"><Button onClick={() => setCurrentStep('welcome')} size="lg" className="w-full bg-gradient-to-r from-[#F4FDFF] to-[#938BA1] text-[#1C448E] hover:shadow-lg hover:shadow-[#938BA1]/20">Try Again <Sparkles className="ml-2 h-4 w-4" /></Button><Button onClick={() => navigate('/support')} variant="outline" className="w-full border-[#F4FDFF]/20 text-[#F4FDFF] hover:bg-[#F4FDFF]/10">Contact Support</Button></div>
              </div>
            </div>
          )}
        </motion.div>
        ...
        {currentStep !== 'success' && currentStep !== 'failed' && currentStep !== 'processing' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-8 text-center">
            <Button variant="ghost" size="sm" onClick={() => toast('Help documentation will open in a new window')} className="text-[#F4FDFF]/30 hover:text-[#F4FDFF]/50"><HelpCircle className="h-4 w-4 mr-2" />Need Help?</Button>
            <p className="text-xs text-[#F4FDFF]/20 mt-2">Our verification process is secure and confidential. Read our <a href="/privacy" className="text-[#938BA1] hover:text-[#F4FDFF] transition-colors">Privacy Policy</a></p>
          </motion.div>
        )}
      </div>
    </div>
